        ...
        if inPay:
            self.add_item(
                discord.ui.TextDisplay(
                    ...
                    f"> Se estiver enfrentando algum problema ou tiver alguma dúvida sobre o sistema de pagamentos, utilize o botão abaixo para acessar nosso servidor de suporte."
                )
            )
        ...
        if user.id == target.id:
            self.add_item(
                discord.ui.TextDisplay(
                    ...
                    f"> Caso ache que isso aconteceu por engano ou tenha alguma dúvida, nossa equipe pode ajudar através do servidor de suporte."
                )
            )
        ...
        if userMoney < self.amount:
            self.add_item(
                discord.ui.TextDisplay(
                    ...
                    f"-# ⤷ Utilize o comando `/ducos atm` para visualizar seu saldo completo.\n\n"
                    f"> Se acreditar que houve algum erro com seu saldo, entre em contato com nossa equipe através do suporte."
                )
            )
...
class Pay(commands.Cog):
    ...
    async def pay(
        ...
        if (
            view.amount is not None
            and view.amount > 0
            and not inPay
            and user.id != target.id
            and userMoney >= view.amount
        ):
